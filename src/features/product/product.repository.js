...
class productRepository{
    async add(productData){
        try{
            //1.Add the new product with categories id's 
            productData.categories=productData.category.trim().split(',');
            console.log(productData);
            const newProduct= new productModel(productData);
            const savedProduct = await newProduct.save();

            //2.update categories collection with documents(contain name and its matching products)
            await categoryModel.updateMany(
                //using $in such that it takes(loops through) all the individual categories present in the array of this product 
                //and then pushes this particular product._id in matching categories products array 
    ...
    async rate(productId,userId,rating){
        try{
            ...
            //3.updating reviews of product
            await productModel.updateOne({_id:productId},{$push:{reviews:{userId:new ObjectId(userId),rating:rating}}});
